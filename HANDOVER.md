...
- **Null-body fix (2026-06-20):** statuses 101/204/205/304 must be returned with a `null` body — building a `NextResponse` with a body for them throws → 500. Surfaced via `register_device_token` (void RPC → 204) and DELETE (204). Verified live. Do not revert to always reading `arrayBuffer()`.
